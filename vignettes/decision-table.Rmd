...
Here is the decision table for the probability the terminal catch exceeds 40:
...
We can use `plot_tradeoff()` to visualize the tradeoff in PNI_75 and mean catch in the long term:
...
In this figure, we see a fairly clear tradeoff between PNI and mean catch. Increasing hatchery production increases catch but also reduces the proportion of natural spawners, especially at low productivity (red). At medium and high productivity levels, the tradeoff occurs at medium to high hatchery production. There is little reduction in PNI between hatchery production of 5 and 10 thousand, but the latter production levels produces slightly higher average catch.
