...
Survival to the smolt stage is density-dependent throught the Beverton-Holt stock-recruit relationship. Smolt production is
...
Harvest $u^\textrm{PT}$ in the pre-terminal fishery, assuming no mark-selective fishing, occurs simultaneously along with marine survival ($\textrm{SAR}$, constant base term). Pre-terminal fishery catch of immature NOS of generation $g$ is calculated as
...
Assuming no mark-selective fishing, the catch of the terminal fishery is calculated from the harvest rate $u^T$:
...
where $p^\textrm{NOS} = \textrm{NOS}/(\textrm{NOS} + \gamma\times\textrm{HOS})$. 
...
$$
p^\textrm{WILD}_g = p^\textrm{NOS}_g \times
\dfrac{(p^\textrm{NOS}_{g-1})^2}
{(p^\textrm{NOS}_{g-1})^2 + 2 p^\textrm{NOS}_{g-1}(1 - p^\textrm{NOS}_{g-1}) +
(1 - p^{\textrm{NOS}}_{g-1})^2}
$$

where the first term is the proportion of natural spawners in the current generation $g$. 
...
Assuming non-assortative mating, the denominator in the second term consists of the proportion of animals whose parents who are both natural origin, mixed origin (one parent in natural origin and the other is hatchery origin), and both hatchery origin.

# Mark-selective fishing

If the mark rate of hatchery fish $m$ is specified to be greater than zero, then mark-selective fishing is implemented for both the pre-terminal and terminal fisheries. 
The mark rate is a proxy for retention and the harvest rate $u$ corresponds to the ratio of the retained catch and abundance of hatchery origin fish.
The harvest rate and mortality will differ between hatchery and natural origin fish when there is no retention on the latter, i.e., $m^\textrm{NOS} = 0$.

Intuitively, fishing effort can increase in a mark-selective fishery compared to non-selective fishery, e.g., if the mark rate is 20 percent, then the fishing effort is 500 percent higher than in a non-selective fishery in order to attain the specified retention quota or bag limit.
Additional mortality can occur for un-marked fish, depending on the proportion of released fish that die ($\delta$).

The instantaneous fishing mortality in the pre-terminal and terminal fishery is

\begin{align}
F^\textrm{PT} &= mE^\textrm{PT} + (1 - m)\delta E^\textrm{PT}\\
F^\textrm{T} &= mE^\textrm{T} + (1 - m)\delta E^\textrm{T}
\end{align}

where $E$ is an index of fishing effort. 

The effort for the pre-terminal and terminal fisheries is obtained from the corresponding $u$ such that

\begin{align}
u^\textrm{PT} &= \dfrac{mE^\textrm{PT}}{F^\textrm{PT} + M^\textrm{SAR}}(1 - \exp(-[F^\textrm{PT}+M^\textrm{SAR}])\\
u^\textrm{T} &= \dfrac{mE^\textrm{T}}{F^\textrm{T}}(1 - \exp(-F^\textrm{T}))
\end{align}

The proportion of released fish that die is $(1 - m)\delta E(1 - \exp(-[F+M])/(F+M)$ and $(1 - m)\delta E(1 - \exp(-F)/F$ in the pre-terminal and terminal fishery, respectively.
