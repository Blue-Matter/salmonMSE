...
$$
\bar{W}^\textrm{nat.}_g = \exp\left(\dfrac{-(\bar{z}^\textrm{nat.}_g-\theta^\textrm{nat.})^2}{2(\omega+\sigma)^2}\right)
$$
...
With a hatchery program, the mean trait value of the progeny in the natural environment is a weighted average between the mean trait value of natural origin spawners in the natural environment (with optimum value $\theta^{nat.}$) and that of hatchery origin spawners in the hatchery environment (with optimum value $\theta^{hat.}$):

$$
\bar{z}^\textrm{nat.}_g = p^\textrm{NOS}_{g-1} \bar{z}^{\prime\textrm{nat.}}_{g-1} + (1 - p^\textrm{NOS}_{g-1}) \bar{z}^{\prime\textrm{hat.}}_{g-1}
$$

where $p^\textrm{NOS} = \textrm{NOS}/(\textrm{NOS} + \gamma\times\textrm{HOS})$. The mean trait value in the hatchery environment $\bar{z}^\textrm{hat.}_g$ is a weighted average of the mean trait value of the natural origin broodtake in the natural environment and hatchery origin broodtake in the hatchery environment:

$$
\bar{z}^\textrm{hat.}_g = p^\textrm{NOB}_{g-1} \bar{z}^{\prime\textrm{nat.}}_{g-1} + (1 - p^\textrm{NOB}_{g-1}) \bar{z}^{\prime\textrm{hat.}}_{g-1}
$$
