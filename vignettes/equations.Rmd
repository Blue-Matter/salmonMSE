...
To generalize for mixed-brood year return, we calculate the probability weighted across brood-years:

$$
p^\textrm{WILD}_y = \sum_a p^\textrm{NOScensus}_{y,a} \times \dfrac{(\sum_{a'} p^\textrm{NOScensus}_{y-a,a'})^2}
...
$p^\textrm{NOScensus}_{y,a} = \dfrac{\textrm{NOS}_{y,a}}{\sum_a{\textrm{NOS}_{y,a}} + \sum_a\textrm{HOS}_{y,a}}$

$p^\textrm{HOScensus}_{y,a} = \dfrac{\textrm{HOS}_{y,a}}{\sum_a{\textrm{NOS}_{y,a}} + \sum_a\textrm{HOS}_{y,a}}$

The summation across dummy age variable $a'$ calculates the total proportion of spawners by origin in year $y-a$, assuming non-assortative mating across age and origin.
...
Intuitively, fishing effort can increase in a mark-selective fishery compared to non-selective fishery.
...
$K^\textrm{HOS,PT} = \dfrac{F^\textrm{kept,PT}_y}{F^\textrm{kept,PT}_y + F^\textrm{rel,PT}_y}(1 - \exp(-v^\textrm{PT}_a[F^\textrm{kept,PT} + F^\textrm{rel,PT}]))N^\textrm{juv,HOS}_{y,a}$.

The corresponding exploitation rates from kept catch and dead discards are
...
for natural and hatchery fish, respectively.
