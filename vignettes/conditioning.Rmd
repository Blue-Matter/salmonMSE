...
For the hatchery-origin population, the juvenile abundance is calculated from an initial equilibrium release assumption $N^\textrm{rel}_{eq}$:

$$
N^\textrm{juv,HO}_{y=1,a} = N^\textrm{rel}_{eq} \times s^\textrm{rel} \times \ell^\textrm{Finit}_a 
$$

For the natural-origin population, two assumptions are available. 

First, the juvenile abundance is in equilibrium corresponding to the observed escapement $E_{y=1}$ in the first year (feasible if there is no hatchery population at the beginning of the model):

$$
N^\textrm{juv,HO}_{y=1,a} = \dfrac{E_{y=1}}{\tau^\textrm{Finit}} \times \ell^\textrm{Finit}_a
$$

where the juvenile abundance is calculated from the ratio of initial escapement and spawner-per-juvenile $\tau^\textrm{Finit} = \sum_a \ell^\textrm{Finit}_a \exp(-v^\textrm{PT}_aF^\textrm{init,PT})\exp(-v^\textrm{T}_aF^\textrm{init,T}) m^\textrm{base}_a p^\textrm{female}$.
...
For example, if the true sampling rate is 0.1, then an initial fit may use $\lambda = 10$, but the CWT can be up-weighted assuming a higher sampling rate of 0.5 ($\lambda = 2$). The analyst should multiply the CWT catch and escapement by 5, or equivalently, multiply the releases by 0.2 with $\lambda = 1$.
