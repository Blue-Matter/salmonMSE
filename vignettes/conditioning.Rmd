...
The abundance of juvenile fish, return, and escapement uses the same exploitation rate, maturity, and natural mortality parameters estimated from the CWT. 
...
where $M^\textrm{egg}_\textrm{min} = -\log(\phi_0^{-1}) - \log(\kappa)$ is the density-independent egg mortality rate when the egg production approaches zero, $\beta \times \textrm{Egg}_y$ is the density-dependent mortality term, and $\delta_y$ is a year-specific deviation.

The Ricker $\alpha$ parameter is $\alpha = \kappa/\phi_0$, where $\kappa$ is productivity (return/spawner) and $\phi_0$ is the equilibrium unfished egg-per-smolt corresponding to the one-to-one return per spawner line, with $\phi_0 = \sum_a \ell_a m^\textrm{base}_a f_a p^\textrm{female}$.
...
The $\beta$ parameter is $\beta = \log(\kappa)/(\phi_0J_0)$, where $J_0$ is the equilibrium juvenile smolt production. 
The equilibrium spawners is $S_0$ and $J_0 = S_0/\phi'_0$, where $\phi'_0 = \sum_a \ell_a m^\textrm{base}_a p^\textrm{female}$ is the equilibrium female spawners per smolt.
