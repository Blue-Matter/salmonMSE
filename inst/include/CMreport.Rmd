...
```{r trace-cr, fig.cap="Wormplots of productivity parameters, scaling parameters and the log-posterior from MCMC iterations after thinning and warmup."}
pars_core <- c("cr", "log_smax", "moadd", "log_FbasePT", "log_FbaseT", "lp")
CM_trace(stanfit, pars_core)
```

```{r trace-cov, fig.cap="Wormplots of coefficients for mortality covariates from MCMC iterations after thinning and warmup."}
pars_cov <- c("^b")
CM_trace(stanfit, pars_cov)
```

```{r trace-vul, fig.cap="Wormplots of fishery vulnerability at age parameters from MCMC iterations after thinning and warmup."}
pars_vul <- c("^logit_vul")
CM_trace(stanfit, pars_cov)
```

```{r trace-var, fig.cap="Wormplots of variance parameters from MCMC iterations after thinning and warmup."}
pars_var <- c("lnE_sd", "wt_sd", "wto_sd", "fanomalyPT_sd", "fanomalyT_sd", "sd_matt")
CM_trace(stanfit, pars_var)
```

```{r pairs-cr, fig.cap="Marginal posterior distributions and correlations of productivity parameters, scaling parameters and the log-posterior (from MCMC iterations after thinning and warmup)."}
...
```{r fit-esc, fig.cap="Observed total escapement (return to river, points with dotted lines) and posterior predicted values (medians and 95% prediction interval)."}
CM_fit_esc(report, d, year)
```

```{r fit-pHOS, fig.cap="Observed pHOS (points with dotted lines) and posterior predicted values (medians and 95% prediction interval)."}
...
```{r SRR, fig.cap="Estimated stock-recruit relationship (medians and 95% prediction interval). Numbers correspond to years of egg production (predicts juvenile production for the following calendar year). Points show annual smolt production (medians)."}
CM_SRR(report, year1)
```
...
```{r smolt, fig.cap="Natural smolt production (outmigrating juveniles, medians and 95% prediction interval)."}
report <- lapply(report, function(i) {
  i$Smolt <- i$N[, 1, 1]
  return(i)
})
...
```{r Meggsurv, fig.cap="Estimated egg-smolt survival of natural origin fish (medians and 95% prediction interval)."}
CM_Megg(report, year1, ci = TRUE, surv = TRUE) +
  coord_cartesian(ylim = c(0, 1))
```

```{r prod, fig.cap="Realized productivity (medians and 95% prediction interval) from annual maturity and juvenile natural mortality values and constant density-dependent egg-juvenile survival. Values are less than 1 when there is insufficient spawners because juvenile natural mortality is high and/or maturity is late."}
CM_prod(report, d, year1)
```

```{r Srep, fig.cap="Realized spawners at replacement (medians and 95% prediction interval) from annual maturity and natural mortality values and constant density-dependent egg-juvenile survival. Values are less than zero when realized productivity is less than 1."}
CM_Srep(report, d, year1)
```

```{r Erep, fig.cap="Realized egg production at replacement (medians and 95% prediction interval) from annual maturity and natural mortality values and constant density-dependent egg-juvenile survival. Values are less than zero when realized productivity is less than 1."}
CM_Srep(report, d, year1, type = "egg")
```

### Juvenile abundance

```{r Njuv, fig.cap="Juvenile abundance at age (medians and 95% prediction interval). Natural origin age 1 fish corresponds to smolt production (outmigrating juveniles)."}
...
```{r M2, fig.cap="Survival to age 2. All fish experience age 1 mortality (first year of marine life stage) and hatchery origin fish may have lower survival if release mortality is specified."}
...
```{r recr, fig.cap="Recruitment at age (mature abundance, medians and 95% prediction interval)."}
