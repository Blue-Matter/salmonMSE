...
#' @slot f_brood Function that calculates the natural origin brood and hatchery origin brood from the escapement (after en-route mortality). Function should be of the form
#'  `function(NO, HO, stray, m) {return(list(NOB, HOB_marked, HOB_unmarked, HOB_stray))}`. Allows for bespoke rules for broodtake.
#' @slot brood_import **Not used if `f_brood` is provided** Matrix by age (length `maxage`) for the number of annual imported hatchery origin broodstock. Egg production is weighted by `fec_brood`.
...
#' @slot pmax_esc **Not used if `f_brood` is provided** Numeric. The maximum proportion of total escapement (after en route mortality) that could be used as broodtake. Set to 1 for AHA compatibility. Default is 0.75.
#' @slot pmax_NOB **Not used if `f_brood` is provided** Numeric. The maximum proportion of the natural origin escapement (after en route mortality and `pmax_esc`) to be used as broodtake. If broodstock
#'  is limited by `pmax_esc < 1`, then this parameter should be 1. Default is 1.
#' @slot ptarget_NOB **Not used if `f_brood` is provided** Numeric. The target proportion of the natural origin broodtake relative to the overall broodtake, assuming the mark rate is 1 and natural origin fish
